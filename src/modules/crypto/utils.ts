...
export async function getKey(secretKey: string): Promise<CryptoKey> {
	const keyData = new TextEncoder().encode(secretKey)
	const hash = await subtle.digest('SHA-256', keyData)

	return subtle.importKey('raw', hash, { length: 256, name: 'AES-GCM' }, false, ['encrypt', 'decrypt'])
}
...
export function combineShuffleString(str1: string, str2: string): string {
	...
	if (j < hiddenChunks.length) {
		result.push(...hiddenChunks.slice(j))
	}

	return result.join('')
}

export function generateArray(length: number, passkey: string): number[] {
...
export function splitZeroWidthCharacters(str: string): { hiddenText: string; originalText: string } {
	...
	return { hiddenText, originalText }
}
