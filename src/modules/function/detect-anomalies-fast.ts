...
type FastDetectionResult = Pick<DetectionSummary, 'max' | 'maxAnomaly' | 'maxNormal' | 'mean' | 'min' | 'minAnomaly' | 'minNormal' | 'stdDev'> & { anomalies: KeyValueData[] }

function detectAnomaliesFast(data: KeyValueData[], threshold: number = 2): FastDetectionResult {
	const n = data.length

	if (n === 0) {
		return { anomalies: [], max: 0, mean: 0, min: 0, stdDev: 0 }
	}
